...
//----------------------------//	
	...
	function userExistancy($email){
		echo "in userExistancy";
		echo $email;
		try 
		{
			echo $sql = "SELECT * FROM `" . DB_PREFIX . "users` WHERE `email`='".$email."'";
			echo "before query execution";
			echo $val=$this->db->get_row($sql,ARRAY_A);		
			return $val;
		}
		catch(Exception $e)
		{
	...
	function searchDoctor($docName,$speciality,$zip,$insuracePlan,$subInsurance,$srchReason,$gender){
		try 
		{
			//echo $gender;
		//	echo $speciality."--".$zip."--".$insuracePlan."--".$subInsurance."--"."--".$gender;
			...
			if($speciality){
				...
				$cond .= "       AND DSPL.`speciality_id` = '".$speciality."' ";
			}
			if($zip){
				$zipstart=$zip;
	...
	function searchDoctorLimit($docName,$speciality,$zip,$insuracePlan,$subInsurance,$srchReason,$gender,$start,$perpage){
		try 
		{
...
//exit;
	...
	function getDetails($useridf){
		try 
		{
			$sql = "SELECT * FROM `" . DB_PREFIX . "doctor_appointments` WHERE `patient_id`='".$useridf."' ";	
			return $this->db->get_results($sql,ARRAY_A);
		}
		catch(Exception $e)
		{

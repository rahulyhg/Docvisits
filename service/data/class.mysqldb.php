...
class mysqldb{
	...
	function AppoinmentCountNew($useridf,$doctor_id){
		try
		{
			$sql = "SELECT doctor_id, count(doctor_id) from `" . DB_PREFIX . "doctor_appointments` WHERE `doctor_id`='".$doctor_id."' AND `patient_id`='".$useridf."' GROUP BY doctor_id";   
			return $this->db->get_results($sql,ARRAY_A);
		}
		catch(Exception $e)
		{
...
//----------------------------//	
	...
	function searchDoctor($name,$speciality,$zip,$insuracePlan,$subInsurance,$srchReason,$gender){
		try 
		{
			//echo $gender;
			//echo $speciality."--".$zip."--".$insuracePlan."--".$subInsurance."--"."--".$gender;
			$cond = "";
			...
			if($zip){
				$zipstart=$zip;
				$cond .= "AND concat( USR.`zipcode`,',', USR.`city`,',', USR.`address`,',',USR.`state` ) like '%".$zipstart."%'";
			}
			else{
				$zipstart=33324;
			...
			if($name){
				$cond .= "       AND concat(USR.`firstname`, ' ',USR.`lastname`) like '%".$name."%'";
			}
			/*if($srchReason){
			...
			USR.`firstname`, 
			       ...
			       USR.`zipcode`, 
			       USR.`description`, 
			       SPL.`id`                         AS specilaityID, 
			       SPL.`name`                       AS specilaityName, 
			       DSPL.`speciality_id`             AS doctor_specilaityID, 
			       (SELECT `name` 
			        FROM   `scad_userimages` 
			        WHERE  `user_id` = USR.`id` 
			               AND `set_profile` = '1') AS imageName, 
			       DINS.`insurance_id`              AS insuranceID, 
			       DINS.`sub_insurance_id`          AS subInsuranceID 
			 FROM  `" . DB_PREFIX . "doctor_speciality` DSPL 
			 LEFT JOIN `" . DB_PREFIX . "users` USR ON DSPL.`doctor_id` = USR.`id`
			 LEFT JOIN `" . DB_PREFIX . "speciality` SPL ON DSPL.speciality_id = SPL.id 
			 LEFT JOIN `" . DB_PREFIX . "doctor_insurance` DINS ON DSPL.`doctor_id` = USR.`id`
			 WHERE  USR.`usertype` = 1  AND USR.`status` = '1' 
			 " . $cond . "
			 GROUP  BY USR.`id` " ;	
			/*$sql .= " GROUP  BY USR.`id` " ;*/	
			//echo $sql;
	...
	function searchDoctorLimit($name,$speciality,$zip,$insuracePlan,$subInsurance,$srchReason,$gender,$start,$perpage){
		try 
		{
			...
			USR.`firstname`, 
			 ...
			 WHERE  USR.`usertype` = 1  AND USR.`status` = '1'  
			 " . $cond . "
			 GROUP  BY USR.`id` ORDER BY `USR`.`id` ASC  LIMIT ".$start." , ".$perpage."" ;	
			//echo $sql;	
			//echo $sql;exit;
			return $this->db->get_results($sql,ARRAY_A);
		}
	...
	function getAllTableData($tbname,$where)
	{
		try
		{
			echo $sql = "SELECT * FROM " . DB_PREFIX . "$tbname WHERE $where";
			echo "before calling sql";
			return $this->db->get_row($sql);
		}
		catch(Exception $e)
		{
	...
	// return $result['results'][0]['geometry']['location'];
		...
		$request_url = "http://maps.googleapis.com/maps/api/geocode/xml?address=".$Address."&sensor=true"."&key=AIzaSyCweoJVrXBCHxWMVqTt3PYdUo8M8AU2Zxc";
	...
	function getIllness($illId){
		try 
		{
			$sql = "SELECT * FROM `" . DB_PREFIX . "illness` WHERE `id`='".$illId."'";	
			return $this->db->get_results($sql,ARRAY_A);
		}
		catch(Exception $e)
		{
	...
	function getDetails($useridf){
		try 
		{
			$sql = "SELECT * FROM `" . DB_PREFIX . "doctor_appointments` WHERE `patient_id`='".$useridf."' GROUP BY doctor_id ORDER BY apnt_date DESC";	
			return $this->db->get_results($sql,ARRAY_A);
		}
		catch(Exception $e)
		{
	...
	function getAppointmentDetailsPatient($useridf,$docId){
		try 
		{
			$sql = "SELECT * FROM `" . DB_PREFIX . "doctor_appointments` WHERE `patient_id`='".$useridf."' and `doctor_id`='".$docId."' ORDER BY id DESC ";	
			return $this->db->get_results($sql,ARRAY_A);
		}
		catch(Exception $e)
		{
	...
	function getPatientAppointmentDetails($useridf){
		try 
		{
			$sql = "SELECT * FROM `" . DB_PREFIX . "doctor_appointments` WHERE `patient_id`='".$useridf."'  ORDER BY id DESC ";	
			return $this->db->get_results($sql,ARRAY_A);
		}
		catch(Exception $e)
		{

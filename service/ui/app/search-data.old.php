...
//functions begin
	...
	function searchDoctor($speciality,$zip,$insuracePlan,$subInsurance,$srchReason,$gender){
			//echo $gender;
			...
			$sql .= " GROUP  BY USR.`id` " ;	
			//echo $sql;exit;
			$result_data['doctorSearch']=$sql;
			echo json_encode($result_data);

	}


	function searchDoctorLimit($speciality,$zip,$insuracePlan,$subInsurance,$srchReason,$gender,$start,$perpage){
		...
		$zipstart=$zip;
			...
			//echo $sql;exit;
						$result_data['doctorSearchLimit']=$sql;
						echo json_encode($result_data);
	}
...
if ($_POST['page']) {
	...
	{
		//echo $element."\n";
		$keypairs = explode("=",$element);
		$key=$keypairs[0];
		$value=$keypairs[1];
		if($key==="docSpeciality"){
			//echo "true-------";
			$docSpeciality=$value;
		}
		if($key==="docZip"){
		...
		if($key==="insuranceSelect"){
			//echo "true-------";
			$insuranceSelect=$value;
		}
		if($key==="subInsuranceSelect"){
			//echo "true-------";
			$subInsuranceSelect=$value;
		}
		if($key==="srchReason"){
			//echo "true-------";
			$srchReason=$value;
		...
		//echo $docSpeciality."------".$docZip."---------".$insuranceSelect."-----------".$subInsuranceSelect."------".$srchReason."------".$gender;

		/*switch($key) {
			case "docSpeciality":$docSpeciality=$value; break;
		}*/
		//echo $docSpeciality
	}
  	$result  = $scad->searchDoctorLimit($docSpeciality, $docZip, $insuranceSelect, $subInsuranceSelect,$srchReason,$gender, $start, $per_page);
	$result1 = $scad->searchDoctor($docSpeciality, $docZip, $insuranceSelect, $subInsuranceSelect,$srchReason,$gender);
	$end  = count($result1);
    $end1 = count($result);

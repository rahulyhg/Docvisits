...
foreach ($res_data as $key => $value) {
    if($value['working_plan'] != 'none'){
        foreach ($days as  $day) {
            ...
            $res_data[$key][$day]['break_time'] = (array_key_exists('breaks',$value)&&array_key_exists($day,$value['breaks'])) ? get_break_time($value['breaks'][$day]) : '';
...
$docName=str_replace('+', " ", $docName);
  $result = $scad->searchDoctorLimit($docName,$srchSpeciality,$docZip,$insuranceSelect,$subInsuranceSelect,$srchReason,$gender,$start,$per_page);
  $result1 = $scad->searchDoctor($docName,$srchSpeciality,$docZip,$insuranceSelect,$subInsuranceSelect,$srchReason,$gender);

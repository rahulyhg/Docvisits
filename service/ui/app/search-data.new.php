...
/*    if ($search == 1) {
	...
	foreach($searchTerms as $searchQuery) {
		$subString=explode("=",$searhcQuery);
		var $string=subString[0];
		var $value=subString[1];
		if($string==="docSpeciality"){
			$docSpeciality=$value;
		} 
		if($string==="gender"){
			$gender=$value;
		}
		if($string==="srchReason"){
			$srchReason=$value;
		}
		if($string==="subInsuranceSelect"){
			$subInsuranceSelect=$value;
		}
		if($string==="insuranceSelect"){
	...
	$subInsuranceSelect=$_POST['subInsuranceSelect'];
	$srchReason=$_POST['srchReason'];
	$gender=$_POST['gender'];
	echo $result  = $scad->searchDoctorLimit($docSpeciality, $docZip, $insuranceSelect, $subInsuranceSelect,$srchReason,$gender, $start, $per_page);
	echo $result1 = $scad->searchDoctor($docSpeciality, $docZip, $insuranceSelect, $subInsuranceSelect,$srchReason, $gender);
	$end  = count($result1);
    $end1 = count($result);	*/
      //$result_data['date_slide'] = $date_slide;
	$result_data['html'] = $docSpeciality.$docZip.$insuranceSelect.$subInsuranceSelect.$srchReason.$gender;

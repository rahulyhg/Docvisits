...
function calendar_html($value,$columns,$Day,$date,$s,$key){
	...
	if($value['working_plan']=='none'){
		...
		for($i=0;$i<$columns;$i++){
			...
			//echo "----------------checking appointment Details------------\n";
			//echo array_key_exists('apntdate',$value);
			//echo "----------------checking appointment Details-----------------\n";
			//echo $date_C."--------------".$day_C."----------\n";
			foreach ($value[$day_C]['working_time'] as $ve_key => $ve_value) {	
				echo "------------checking for ".$date_C."and time".$ve_value."---------\n";
				//echo "--------------inside foreach----Key--------".$ve_key."=".$ve_value."-----\n";			
				if($ve_key>4){
			...
			} elseif(is_array($value['vacation_date']) && in_array($date_C, $value['vacation_date'])&&in_array($ve_value, $value['vacation_time'][$date_C])){
				echo "vacation";
				...
				if(is_array($value['apntdate'])){
				//echo "else if 4";
					//print_r($value['apntdate']);
					//echo $date_C;
				//echo "in appointment";
					foreach ($value['apntdate'] as $key => $value12) {
						if($date_C===$value12['apnt_date']) {
							//echo "Cheking appointments".$date_C."-----------\n";
						//echo $ve_value;
							//$value12['apnt_starttime'];
							$newTime=date_format(date_create_from_format('H:i a', $ve_value), 'H:i:s');
							if($newTime==$value12['apnt_starttime']) {
								echo "booked";
							...
							//echo "In ELSE --- No Need to check appointments".$date_C."-----------\n";
						}
					# code...
					}
...
$srchSpeciality=$_GET['srchSpeciality'];
$srchZipcode=0;
...
$start        = $page * $per_page;
  //echo "----".$docName."------";
$result = $scad->searchDoctorLimit($docName,$srchSpeciality,$srchZipcode,$insuranceSelect,$subInsuranceSelect,$srchReason,$gender,$start,$per_page);
$result1 = $scad->searchDoctor($docName,$srchSpeciality,$srchZipcode,$insuranceSelect,$subInsuranceSelect,$srchReason,$gender);
  //print_r($result1);
$end  = count($result1);
$end1 = count($result);

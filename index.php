...
if (isset($_SERVER['HTTP_ORIGIN'])) {
	...
	//echo "hello word";exit;
   	...
   	if(!$_SESSION['userID']){ 
   		...
   		if($_SESSION['userType']==2){
   			...
   			$app->render('patient-profile_new.php');
   ...
   $app->post('/store-zipcode', function () use ($app) {	
	...
	//authenticate_package($app);
   	...
   	$app->render('store-zipcode.php',$postData); 
...
/*$app->get('/admin/data', function () use ($app) {
	...
	$app->get('/patient/check-in-online-form', function () use ($app) {
...
// end of admin panel functions
	...
	$app->get('/join/learnmorep', function () use ($app) {

		authenticate_package($app);

		include("conf/config.inc.php");

		$app->render('whyjoinP.php');
	...
	$app->post('/doctorAverageReview', function () use ($app) {	
		...
		$app->render('doctor-average-review.php',$postData); 
	...
	$app->get('/sub-header', function () use ($app) {	

	//authenticate_package($app);

		$request	= \Slim\Slim::getInstance()->request();

		$app->render('header_sub.php'); 
	...
	$app->get('/search(/:seachData)', function ($seachData=NULL) use ($app) {
		...
		/*$app->render('search_new.php',$data);*/
	...
	$app->get('/patient_settings', function () use ($app) {
		...
		$app->render('patient_settings_new.php');
...
//write by arun end
	...
	$app->get('/appointment/fixing/:apntDatetime/:ins/:subins/:search', function ($apntDatetime,$ins,$subins,$search) use ($app) {
		...
		$app->render('schedule-appointment-new.php',$data);
	...
	$app->get('/distance/', function ($doctorID) use ($app) {

		authenticate_package($app);

		$app->render('distance.php');

	});

	$app->get('/rating-doc/:doctorID', function ($doctorID) use ($app) {

		authenticate_package($app);

		$data['doctorID'] = $doctorID;						 

		$app->render('rating-data-doc.php',$data);
